...
def test_arc_manager_basic():
    ...
    # prepare store [1, 2]
    prepare_store_output = arc_manager.prepare_store(to_keys([1, 2]))
    verify_store_output(
        prepare_store_output,
        ExpectedPrepareStoreOutput(
            keys_to_store=[1, 2],
...
def test_arc_manager_t1_to_t2_promotion():
    ...
    arc_manager.prepare_store(to_keys([1]))
...
def test_arc_manager_eviction_with_load():
    ...
    prepare_store_output = arc_manager.prepare_store(to_keys([1, 2, 3, 4]))
    verify_store_output(
        prepare_store_output,
        ExpectedPrepareStoreOutput(
    ...
    arc_manager.complete_store(to_keys([1, 2, 3, 4]))

    # prepare load [2, 3] (increases ref_cnt)
    prepare_load_output = arc_manager.prepare_load(to_keys([2, 3]))
    verify_load_output(prepare_load_output, [1, 2])

    # prepare store [5, 6, 7] with [2, 3] being loaded
    # should fail because [2, 3] have ref_cnt > 0
    assert arc_manager.prepare_store(to_keys([5, 6, 7])) is None

    # complete load [2, 3]
    arc_manager.complete_load(to_keys([2, 3]))

    # now prepare store [5, 6, 7] should succeed
    # ARC will evict blocks one at a time from T1 as needed
    prepare_store_output = arc_manager.prepare_store(to_keys([5, 6, 7]))
    assert prepare_store_output is not None
...
def test_arc_manager_t1_t2_eviction_policy():
    ...
    arc_manager.prepare_store(to_keys([1, 2, 3, 4]))
    arc_manager.complete_store(to_keys([1, 2, 3, 4]))

    # promote blocks 3, 4 to T2 by touching them
    arc_manager.touch(to_keys([3, 4]))
    ...
    output = arc_manager.prepare_store(to_keys([5]))
...
def test_arc_manager_ghost_list_bounds():
    ...
    for i in range(3, 20):
        arc_manager.prepare_store(to_keys([i]))
        arc_manager.complete_store(to_keys([i]))
...
def test_arc_manager_touch_ordering():
    ...
    arc_manager.touch(to_keys([1, 3, 4]))
    ...
    prepare_store_output = arc_manager.prepare_store(to_keys([5]))
    verify_store_output(
        prepare_store_output,
        ExpectedPrepareStoreOutput(
            keys_to_store=[5],
...
def test_arc_manager_failed_store():
    ...
    assert evicted_key in arc_policy.b1
...
def test_arc_manager_full_scenario():
    ...
    prepare_store_output = arc_manager.prepare_store(to_keys([3, 4, 5]))
    assert prepare_store_output is not None
    assert len(prepare_store_output.evicted_keys) == 1
    arc_manager.complete_store(to_keys([3, 4, 5]))

    # promote some blocks to T2
    arc_manager.touch(to_keys([2, 3]))
    ...
    prepare_store_output = arc_manager.prepare_store(to_keys([6]))
    assert prepare_store_output is not None
