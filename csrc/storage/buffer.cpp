...
static std::pair<void*, size_t> get_thread_local_pinned(size_t required_bytes,int numa_node=-1) {
    if (!t_pinned_ptr || t_pinned_size < required_bytes) {
        if (t_pinned_ptr) {
            ...
            std::cerr << "[WARN] Thread " << std::this_thread::get_id()
                      << " existing pinned buffer too small ("
                      << t_pinned_size << " bytes), reallocating "
...
int get_gpu_numa_node(int device_id) {
    ...
    cuDeviceGetAttribute(&numa_node,
